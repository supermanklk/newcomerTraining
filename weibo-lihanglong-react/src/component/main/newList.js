        ...
        return (
            <div className="newsList">
                {newsList.map((item, index) => {
                    return <div className="news" key={index} onClick={() => this.handleClick(index)}>
                        <div className="left">
                            <div className={["num-icon", (index < 4) ? " wei-style" : null].join('')}>{index > 0 ? index : "'"}</div>
                            <div className="content">{item.content}</div>
                            <div className="num" >{item.num}</div>
                        </div>
                        <div className={["tag", (item.tag === "热") ? " re" : null, (item.tag === "沸") ? " fei" : null, (item.tag === "新" ? " xin" : null)].join('')}>
                            {item.tag}
                        </div>
                    </div>
                })}
